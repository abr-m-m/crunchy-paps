-- 18 sep 2026 — Las fechas «de día» se guardan a mediodía de CDMX.
--
-- El checkout mandaba la fecha de entrega como new Date('AAAA-MM-DD').toISOString(): medianoche UTC.
-- Con la base en UTC, fecha_entrega::date daba el día elegido; con la base en CDMX
-- (20260930000000) da el día anterior: cola_armado, lineas_armado_interno y reparto_del_dia
-- buscaban el pedido del lunes en el domingo. La app, que lee con new Date() en el navegador,
-- ya lo enseñaba un día antes. Medido en producción: 67 de 70 fecha_entrega a 00:00 UTC; las
-- otras 3 ya a mediodía (editar el pedido guarda T12:00 local) o con la hora de la venta.
--
-- 1. ordenes.fecha_entrega: 00:00 UTC → 12:00 CDMX del mismo día (+18 h). fecha_actualizacion
--    no se toca (su trigger se apaga durante el update). Caja y puntos solo reaccionan a cambios
--    de estatus, que aquí no hay.
-- 2. cupones.vigencia_*: 00:00 UTC → 00:00 CDMX del mismo día (+6 h). El servidor compara con
--    now() (6 h de diferencia en el corte); la pantalla de edición ya no enseña un día antes.
--    guardar_cupon, con la base en CDMX, ya guarda las nuevas a 00:00 CDMX.
-- 3. crear_pedido: una fechaEntrega a 00:00 UTC exactas (app vieja en caché) pasa a mediodía
--    de CDMX. La app nueva manda AAAA-MM-DDT12:00:00-06:00.

begin;

alter table public.ordenes disable trigger trg_ordenes_actualizacion;
update public.ordenes set fecha_entrega = fecha_entrega + interval '18 hours'
 where (fecha_entrega at time zone 'UTC')::time = '00:00'
returning consecutivo, fecha_entrega;
alter table public.ordenes enable trigger trg_ordenes_actualizacion;

alter table public.cupones disable trigger trg_cupones_actualizacion;
update public.cupones set
  vigencia_inicio = case when (vigencia_inicio at time zone 'UTC')::time = '00:00' then vigencia_inicio + interval '6 hours' else vigencia_inicio end,
  vigencia_fin    = case when (vigencia_fin    at time zone 'UTC')::time = '00:00' then vigencia_fin    + interval '6 hours' else vigencia_fin    end
 where (vigencia_inicio at time zone 'UTC')::time = '00:00' or (vigencia_fin at time zone 'UTC')::time = '00:00'
returning codigo, vigencia_inicio, vigencia_fin;
alter table public.cupones enable trigger trg_cupones_actualizacion;
