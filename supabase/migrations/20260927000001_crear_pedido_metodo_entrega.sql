-- 20260927000001_crear_pedido_metodo_entrega.sql — Reparto P1, Tarea 4.
-- Cuerpo volcado de producción el 17 sep 2026 (idéntico al de staging: diff vacío, 344 líneas)
-- más DOS líneas: la columna metodo_entrega en el INSERT de ordenes. Depende de
-- 20260927000000_reparto.sql (crea la columna). Nada más cambia.
...
-- Mismos permisos que tenía (leídos de pg_proc.proacl el 17 sep): anon, authenticated, service_role.
revoke all on function public.crear_pedido(jsonb) from public;
grant execute on function public.crear_pedido(jsonb) to anon, authenticated, service_role;
