-- ============================================================================
--  Precio propio por caja (13 sep 2026). Decisión de Abraham: «columnas por
--  producto». Hasta hoy la caja no existía en la base: la línea se guardaba
--  como piezas (cantidad = cajas × piezas) a precio de pieza del canal, y
--  crear_pedido recotizaba cantidad × precio. Con esto:
--
--    · productos.precio_caja_3 / _6 / _12 (NULL o 0 = sin precio propio: la
--      caja cuesta piezas × precio del canal, como hasta hoy).
--    · ordenes_detalle.piezas_por_caja: cuántas piezas trae la caja de esa
--      línea (NULL = se vendió por pieza). La cantidad sigue en PIEZAS, así que
--      el descuento de lote (piezas × gramos), el kárdex y los mínimos de
--      mayoreo no cambian.
--    · crear_pedido lee `caja` en cada línea; si trae 3/6/12 y el producto
--      tiene precio propio para esa caja, cobra cajas × precio_caja (sin
--      descuento_pct: el precio de caja ya es el precio). Si no, como antes.
--
--  ORDEN: primero desplegar la app (manda `caja` y calcula igual que aquí),
--  después esta migración, y al final poner precios en las columnas. Si se
--  ponen precios antes de desplegar la app, la app cotiza piezas × precio y el
--  servidor cajas × precio_caja: `precio_cambiado` en cada pedido con caja.
--
--  Staging primero. Verificar:
--    select sabor, presentacion, precio_tienda, precio_caja_3, precio_caja_6, precio_caja_12
--      from productos where presentacion ilike '%50g%' order by sabor;
--  Reversible: volver a crear crear_pedido con la migración 20260917000000 y
--    alter table productos drop column precio_caja_3, drop column precio_caja_6, drop column precio_caja_12;
--    alter table ordenes_detalle drop column piezas_por_caja;
-- ============================================================================

alter table public.productos
  add column if not exists precio_caja_3  numeric(10,2),
  add column if not exists precio_caja_6  numeric(10,2),
  add column if not exists precio_caja_12 numeric(10,2);

comment on column public.productos.precio_caja_3  is 'Precio de la caja de 3 piezas (tienda y mayorista). NULL o 0 = piezas × precio del canal.';
comment on column public.productos.precio_caja_6  is 'Precio de la caja de 6 piezas. NULL o 0 = piezas × precio del canal.';
comment on column public.productos.precio_caja_12 is 'Precio de la caja de 12 piezas. NULL o 0 = piezas × precio del canal.';

alter table public.ordenes_detalle
  add column if not exists piezas_por_caja integer;

comment on column public.ordenes_detalle.piezas_por_caja is 'Si la línea se vendió por caja: piezas por caja (3, 6, 12). cantidad sigue en piezas.';

-- ── crear_pedido: igual que 20260917000000, más la caja ──────────────────────
-- Cambios marcados con «CAJA». El resto es el cuerpo vigente, copiado tal cual.
...
BEGIN
  ...
  FOR v_producto IN SELECT * FROM jsonb_array_elements(p_data->'productos') LOOP
    ...
    ELSE
      IF v_pcaja IS NOT NULL AND v_caja > 0 AND v_cant > 0 AND MOD(v_cant, v_caja) = 0 THEN
  ...
  FOR v_producto IN SELECT * FROM jsonb_array_elements(v_calc) LOOP
    INSERT INTO ordenes_detalle (
      ...
      NULL,
      NULLIF(COALESCE((v_producto->>'caja')::INTEGER, 0), 0)
    );
...
comment on function public.crear_pedido(jsonb) is
  'Crea un pedido con sus líneas. El servidor recotiza cada línea según el nivel de quien pide; desde el 13 sep 2026 una línea con `caja` (3/6/12) y precio propio en productos.precio_caja_N se cobra cajas × precio_caja y guarda piezas_por_caja. Exige sesión; el uso del cupón va en la misma transacción.';
