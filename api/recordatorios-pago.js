...
module.exports = async (req, res) => {
  ...
  if (CRON_SECRET) {
    ...
    const url  = req.url || '';
    const okAuth = auth === 'Bearer ' + CRON_SECRET || url.includes('secret=' + CRON_SECRET);
  ...
  try {
    ...
    const q = await supa(
      ...
      '&select=id,consecutivo,nombre_cliente,telefono_cliente,total,fecha_orden,ultimo_recordatorio_pago,recordatorios_pago_enviados,tipo_interno' +
    ...
    const dryRun = (req.url || '').includes('dry=1');

    for (const o of candidatos) {
      const to = fmtTel(o.telefono_cliente);
